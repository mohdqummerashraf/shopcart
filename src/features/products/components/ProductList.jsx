import {FormControl, Grid, IconButton, Pagination,InputLabel, MenuItem, Select, Stack, Typography, useMediaQuery, useTheme } from '@mui/material'
    ...
    return (
        <>
            {productFetchStatus === 'pending' ? (
                <Stack width={is500 ? "35vh" : '25rem'} height={'calc(100vh - 4rem)'} justifyContent={'center'} marginX={'auto'}>
                ...
                <>
                    <motion.div
                        style={{
                            position: "fixed", backgroundColor: "white", height: "100vh", padding: '1rem',
                            overflowY: "scroll", width: is500 ? "100vw" : "30rem", zIndex: 500
                        }}
                        ...
                        <Stack mb={'5rem'} sx={{ overflowY: "scroll" }}>
                            ...
                            <Stack rowGap={2} mt={4}>
                                {["Totes", "Backpacks", "Travel Bags", "Hip Bags", "Laptop Sleeves"].map((item) => (
                                    <Typography key={item} sx={{ cursor: "pointer" }} variant='body2'>{item}</Typography>
                                ))}
                            </Stack>
                    ...
                    <Stack mt={2} direction={'row'} justifyContent={'space-between'} alignItems={'center'}>
                        <Typography variant='h5'>{totalResults} Results</Typography>
                            ...
                            <FormControl size="small">
                                ...
                                >
                                    ...
                                    {sortOptions.map((opt, i) => (
                                        <MenuItem key={i} value={opt}>{opt.name}</MenuItem>
                                    ))}
                                </Select>
                            </FormControl>
                        </Stack>
                    ...
                    <Grid container spacing={2} mt={1}>
                        {products.map((product) => (
                            <Grid item key={product._id} xs={6} sm={6} md={4} lg={3} xl={2}>
                                <ProductCard product={product} handleAddRemoveFromWishlist={handleAddRemoveFromWishlist} />
                            </Grid>
                        ))}
                    </Grid>

                    <Stack mt={5} justifyContent={'center'} alignItems={'center'} direction={'row'}>
                        <Pagination count={Math.ceil(totalResults / ITEMS_PER_PAGE)} page={page} onChange={(e, val) => setPage(val)} />
                    </Stack>
                </>
            )}
        </>
